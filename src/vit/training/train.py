...
from vit.config.config import get_config
from vit.data.dataset import get_train_dataset, get_val_dataset
...
def train_model(config=None, use_wandb=True):
    ...
    if use_wandb:
        ...
        wandb_run = init_wandb(wandb_config, run_name='vit-training', model_type='vit')
        if wandb_run:
            wandb.config.update({
